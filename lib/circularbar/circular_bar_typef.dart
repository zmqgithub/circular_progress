...
class CircularProgressf extends StatefulWidget{
  ...
  final Color color;
  CircularProgressf({@required this.size, this.backgroundColor = Colors.grey, this.color = Colors.blue});
...
class _CircularProgress extends State<CircularProgressf> with SingleTickerProviderStateMixin{
...
class CircularCanvas extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    ...
    paint..shader = new LinearGradient(colors: [color.withOpacity(0.5), color]
    ,begin: Alignment.topCenter, end: Alignment.bottomCenter).createShader(rect);

    paint..style = PaintingStyle.stroke;
    canvas.drawCircle(new Offset(size.width/2, size.height/2), size.width/2-strokeWidth/2, paint..color = color.withOpacity(0.3));
